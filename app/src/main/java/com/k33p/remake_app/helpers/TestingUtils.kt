...
fun initMaskArray(context: Context) : ArrayList<Bitmap> {
...
// For testing purpose only
fun dummyParseBitmapsAndBoxes(context: Context) : BoxesAndBitmapsContainer {
    ...
    for (i in 0 until jbox.length()) {
        ...
        boxesAndBitmapsContainer.add(decodedByte, box)
    }
    Log.i(TAG, "Lengths: bitmaps and boxes: ${boxesAndBitmapsContainer.size}")
    return boxesAndBitmapsContainer
}
